...
```http
POST /v1/notifications/settings/reset
Authorization: Bearer <jwt_token>
```
...
The DOPE API includes a comprehensive notification system that automatically handles user interactions and respects user notification preferences.

### Automatic Notifications

The following user actions automatically trigger notifications:

- **Likes**: When a user likes a post, the post author receives a notification
- **Comments**: When a user comments on a post, the post author receives a notification  
- **Reposts**: When a user reposts content, the original author receives a notification
- **Tips**: When a user sends a tip, the recipient receives a notification
- **Donations**: When a user sends a donation (non-anonymous), the recipient receives a notification
- **Follows**: When a user follows another user, the followed user receives a notification
- **New Posts**: When a user creates a post, their followers receive notifications
...
- **In-App**: Notifications displayed within the application (coming soon)
- **Push**: Push notifications to mobile devices (coming soon)

### Notification Preferences

Users can control their notification preferences through the `/v1/notifications/settings` endpoint. Each notification type can be enabled/disabled for each channel.
