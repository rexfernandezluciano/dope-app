...
const LoginPage = () => {
  ...
  const validateTfaCode = useCallback((code) => {
    if (!code) {
  ...
  }, [formData, showTfa, validateEmail, validatePassword, validateTfaCode]);
  ...
  }, [formData, showTfa, validateForm, navigation]);
  ...
  const handleBackFromTfa = useCallback(() => {
    setShowTfa(false);
  ...
  const handleTfaCodeChange = useCallback((text) => {
  ...
  }, [updateFormData, loading, handleLogin]);
  ...
  const canSubmit = showTfa 
    ? formData.tfaCode.length === TFA_CODE_LENGTH && !loading
    : formData.email.trim() && formData.password && !loading;
      ...
      >
        ...
        >
          ...
          >
            ...
            >
              {loading ? (
                <ActivityIndicator color="#0069b5" />
              ) : (
                showTfa ? "Verify Code" : "Sign In"
              )}
            </Button>
